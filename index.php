...
<body class="text-slate-800 hero-gradient min-h-screen">
    ...
    <!-- Hero Section -->
    <header class="pt-32 pb-16 md:pt-48 md:pb-24 px-6 text-center">
        <div class="container mx-auto">
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-6 md:mb-8 animate-bounce">
                <i class="fas fa-fire"></i> New Season Offers
            </div>
            <h1 class="text-4xl md:text-7xl font-black text-slate-900 mb-6 md:mb-8 tracking-tight leading-tight italic px-2">
                Hel <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-sky-500">PUBG UC</span> <br class="hidden md:block"> Si Degdeg Ah
            </h1>
            <p class="text-[15px] md:text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed mb-10 md:mb-12 px-4">
                Goobta ugu kalsoonida badan ee PUBG Mobile. Waxaan bixinaa adeeg degdeg ah oo hufan.
            </p>
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6">
                <a href="#packages" class="w-full sm:w-auto btn-gradient text-white px-8 py-4 md:px-10 md:py-4 rounded-2xl font-extrabold shadow-lg shadow-blue-500/20 text-base md:text-lg">
                    Browse Packages
                </a>
                <a href="#how-it-works" class="w-full sm:w-auto bg-white border border-slate-200 text-slate-700 px-8 py-4 md:px-10 md:py-4 rounded-2xl font-extrabold hover:bg-slate-50 transition-colors text-base md:text-lg">
                    How it Works
                </a>
            </div>
        </div>
    ...
    <!-- Packages Section -->
    <section id="packages" class="py-12 md:py-24 px-4 md:px-6 bg-white/50">
        <div class="container mx-auto">
...
?>
                    <div class="group relative <?php echo $is_popular ? 'popular-border' : ''; ?>">
                        <div class="h-full glass-card rounded-2xl md:rounded-3xl p-5 md:p-8 flex flex-col transition-all md:group-hover:-translate-y-2 md:group-hover:shadow-2xl md:group-hover:shadow-blue-500/10">
                            <?php if ($is_popular): ?>
                                <div class="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-blue-600 to-sky-500 text-white text-[9px] md:text-[10px] font-black uppercase tracking-widest px-3 md:px-4 py-1.5 rounded-full shadow-lg whitespace-nowrap z-10">
                                    <i class="fas fa-star mr-1"></i> Popular
                                </div>
                            <?php
        endif; ?>

                            <div class="flex flex-col sm:flex-row items-center justify-between mb-4 md:mb-8 gap-3 sm:gap-0">
                                <div class="w-12 h-12 md:w-14 md:h-14 bg-blue-50 rounded-xl md:rounded-2xl flex items-center justify-center text-blue-600 text-xl md:text-2xl group-hover:scale-110 transition-transform">
                                    <i class="fas fa-bolt"></i>
                                </div>
                                <div class="text-center sm:text-right">
                                    <span class="block text-2xl md:text-3xl font-black text-slate-900 leading-none mb-1 md:mb-0"><?php echo $row['uc_amount']; ?></span>
                                    <span class="text-[10px] md:text-xs font-bold text-blue-500 uppercase tracking-widest leading-none">UC</span>
                                </div>
                            </div>

                            <div class="text-2xl md:text-4xl font-black text-slate-900 mb-6 md:mb-8 text-center sm:text-left">$<?php echo number_format($row['price'], 2); ?></div>

                            <ul class="hidden sm:block space-y-4 mb-10 flex-grow">
                            ...
                            <button onclick="<?php echo $is_logged_in ? "selectPackage('{$row['uc_amount']}')" : "redirectToLogin('{$row['uc_amount']}')"; ?>" 
                                    class="w-full mt-auto py-3 md:py-4 rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[11px] md:text-sm transition-all
...
?>
    ...
    <section id="how-it-works" class="py-12 md:py-24 px-4 md:px-6">
        <div class="container mx-auto">
            <div class="max-w-4xl mx-auto glass-card rounded-3xl md:rounded-[40px] p-6 md:p-20 relative overflow-hidden">
